...
class TaskScheduler:
    ...
    def setup_weekly_tasks(self):
        ...
        # Facebook post every Tuesday and Thursday at 2:00 PM
        schedule.every().tuesday.at("14:00").do(
            self.run_facebook_post,
            name="facebook_post_tuesday"
        )
        self.jobs.append("facebook_post_tuesday")

        schedule.every().thursday.at("14:00").do(
            self.run_facebook_post,
            name="facebook_post_thursday"
        )
        self.jobs.append("facebook_post_thursday")

        # Instagram post every Wednesday and Friday at 3:00 PM
        schedule.every().wednesday.at("15:00").do(
            self.run_instagram_post,
            name="instagram_post_wednesday"
        )
        self.jobs.append("instagram_post_wednesday")

        schedule.every().friday.at("15:00").do(
            self.run_instagram_post,
            name="instagram_post_friday"
        )
        self.jobs.append("instagram_post_friday")
...
"""
    ...
    def run_facebook_post(self, name: str = "facebook_post"):
        """Schedule/create Facebook post"""
        self.logger.info(f"Running {name}")

        try:
            from skills.social_media_poster import SocialMediaPoster, schedule_business_post
            ...
            if post_result['status'] == 'success':
                # Create approval request
                poster = SocialMediaPoster(str(self.project_path))
                approval_result = poster.create_approval_request(
                    content=post_result['content'],
                    platforms=['facebook']
                )
                ...
                self.logger.info(f"Facebook post created for approval")

        except Exception as e:
            self.logger.error(f"Error in Facebook post: {e}")

    def run_instagram_post(self, name: str = "instagram_post"):
        """Schedule/create Instagram post"""
        self.logger.info(f"Running {name}")
        ...
        try:
            ...
            # Instagram requires an image - use a placeholder or generate one
            ...
            if post_result['status'] == 'success':
                ...
                self.logger.info(f"Instagram post created for approval")

        except Exception as e:
            self.logger.error(f"Error in Instagram post: {e}")
