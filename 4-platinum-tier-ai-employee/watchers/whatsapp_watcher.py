...
class WhatsAppWatcher(BaseWatcher):
    ...
    def _initialize_playwright(self):
        ...
        try:
            ...
            lock_file = self.session_path / 'SingletonLock'
            if lock_file.exists():
                self.logger.warning(f"Removing stale lock file: {lock_file}")
                try:
                    lock_file.unlink()
                except:
                    pass
            ...
            self.logger.warning("WhatsApp watcher will run in limited mode without browser")
