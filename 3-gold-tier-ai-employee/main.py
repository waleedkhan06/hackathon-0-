"""
Main script to run the Gold Tier AI Employee system
...
class GoldTierAIEmployee:
    """Main system coordinator for Gold Tier AI Employee"""
    ...
    def _setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(log_dir / f'gold_tier_ai_employee_{time.strftime("%Y%m%d")}.log'),
    ...
    def _initialize_components(self):
        ...
        self.facebook_watcher = FacebookWatcher(project_path_str, check_interval=120)
        self.instagram_watcher = InstagramWatcher(project_path_str, check_interval=120)
        self.linkedin_watcher = LinkedInWatcher(project_path_str, check_interval=120)
    ...
    def start_watchers(self):
        ...
        # Start Facebook watcher in thread
        self.facebook_watcher_thread = threading.Thread(
            target=self.facebook_watcher.run,
            daemon=True,
            name="FacebookWatcher"
        )
        self.facebook_watcher_thread.start()

        # Start Instagram watcher in thread
        self.instagram_watcher_thread = threading.Thread(
            target=self.instagram_watcher.run,
            daemon=True,
            name="InstagramWatcher"
        )
        self.instagram_watcher_thread.start()

        # Start LinkedIn watcher in thread
        self.linkedin_watcher_thread = threading.Thread(
            target=self.linkedin_watcher.run,
            daemon=True,
            name="LinkedInWatcher"
        )
        self.linkedin_watcher_thread.start()
    ...
    def start(self):
        ...
        self.logger.info("=" * 50)
        self.logger.info("Starting Gold Tier AI Employee System")
        self.logger.info("=" * 50)
    ...
    def stop(self):
        ...
        self.logger.info("Stopping Gold Tier AI Employee System...")
        self.running = False
...
def main():
    ...
    system = GoldTierAIEmployee(str(project_root))
    system.start()
